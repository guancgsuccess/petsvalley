...
<head>
    ...
    <script>
        var url_getmessage = '${pageContext.request.contextPath}/message/get';
        var url_getmessagelist ='${pageContext.request.contextPath}/message/getAllByPageManage';
        //页面加载后，首先加载所有的消息数据
        window.onload=function() {
            refreshMessageList(url_getmessagelist);
        ...
        //加载消息数据
        var xhr = null;
         function refreshMessageList(url_getmessagelist) {
            var objArr = null;
            ...
            xhr.open("GET", url_getmessagelist, true);
            xhr.setRequestHeader("Content-Type", "application/json;charset=utf-8");
            xhr.onreadystatechange = loadMessages;
            xhr.send();
        }
       function delforMessage(obr1){
            var url_delmessage = '${pageContext.request.contextPath}/message/delete/msgid/'+obr1;
            if (window.XMLHttpRequest) {
                xhr = new XMLHttpRequest();
            } else {
                xhr = new ActiveXObject('Microsoft.XMLHTTP');
            }
            xhr.open("GET", url_delmessage, true);
           //xhr.setRequestHeader("X-Requested-With","XMLHttpRequest");
            xhr.setRequestHeader("Content-Type", "application/json;charset=utf-8");
           // xhr.onreadystatechange = refreshMessageList(url_getmessagelist) ;
            xhr.send();
           location.reload();
        }
       function loadMessages() {
            if (xhr.readyState == 4 && xhr.status == 200) {
                ...
                for (var i = 0; i < objArr.length; i++) {
                    tbody.innerHTML += ('<tr><td>' + objArr[i].systemMessageId + '</td>' +
                        '<td >' + objArr[i].systemMessageTitle + '</td>' +
                        '<td >' + objArr[i].systemMessageContent + '</td>' +
                        '<td>' +strFormat(objArr[i].systemMessageTime) +'</td>' + '<td ><a style="cursor: pointer"onclick="getCustomerById(this)"data-toggle="modal" data-target="#customerDetail">' +objArr[i].systemMessageSpare   +'</a></td>' +
                        '<td><a id="btnDel"  style="cursor: pointer" onclick="delforMessage('+ objArr[i].systemMessageId +')">删除</a>  &nbsp;&nbsp;&nbsp;&nbsp;  ' +
                        ' <a id="btnModif" name="' + objArr[i].systemMessageContent+""+'"  style="cursor: pointer" onclick="get33Message(this)" data-toggle="modal" data-target="#modalUpdateEmp">查看</a></td></tr>');
                }
                //页码渲染href="${pageContext.request.contextPath}/message/delete/msgid/ '+ objArr[i].systemMessageId +')"
                var pagearea=document.getElementById("pagearea");
                pagearea.innerHTML="";
                if(page.currentPageCode!=1){
                    var url_page=url_getmessagelist+"/page/"+(page.currentPageCode-1);
                    var href='javascript:refreshMessageList("'+url_page+'")';
        ...
        function get33Message(msg) {
            document.getElementById("myMessage").innerText=msg.name;
        }

        function  href2page() {
            var url2page=url_getmessagelist+"/page/"+(document.getElementById("topage").value);
            refreshMessageList(url2page);
        }
        ...
        function selectbydate() {
            ...
            var url_getmessagelistbydate ='${pageContext.request.contextPath}/message/getAllByPagedateManage/?from1='+from1+'&to='+to;
            refreshMessageList(url_getmessagelistbydate);
        }

        function addpet() {
            var senduserid= document.getElementById("senduserid").value;
            var sendtitle= document.getElementById("addpetgender").value;
            var sendcontent= document.getElementById("addpetage").value;
            if(senduserid==""||sendtitle==""||sendcontent==""){
                alert("信息不完整哦");
                return;
            }else {
                var url_addpet= '${pageContext.request.contextPath}/message/insert';
                if (window.XMLHttpRequest) {
                    xhr = new XMLHttpRequest();
                } else {
                    xhr = new ActiveXObject('Microsoft.XMLHTTP');
                }
                var fd="systemMessageSpare="+senduserid+"&systemMessageTitle="+sendtitle+"&systemMessageContent="+sendcontent;
        ...
        <div style="display: inline-block;width:79% !important;margin-right: -16%">
            <div class="col-md-4 column" style="background-color:whitesmoke;margin-top: 40px;width: 79%">
                <div style="margin-top: 25px;">
                    ...
                    <label><a style="cursor: pointer" onclick="addpetdiv()" data-toggle="modal" data-target="#addpetdiv">发送消息</a></label>
                </div>
            </div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                        <div class="form-group">
                           <table>
                               <tr>
                                   <td style="text-indent: 50px" id="myMessage"></td>
                               </tr>
                           </table>
                        </div>
                        <div class="form-group">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="form-group" style="text-align: center">
                        <form>
                            接收用户:<input type="text"  placeholder="请输入接收用户的编号" id="senduserid" class="inpput"/><br/>
                            消息标题:<input type="text"  placeholder="请输入消息标题" id="addpetgender" class="inpput"/><br/>
                            消息内容:<input  type="text" placeholder="请输入详细信息" id="addpetage" class="inpput"/><br/>
                        </form>
                    </div>
                    <div class="form-group">
                        <div class="col-sm-offset-2 col-sm-10" style="text-align: center">
                            <button type="button" class="btn btn-default" data-dismiss="modal">关闭</button><button type="button" onclick="addpet()" class="btn btn-primary">发送</button>
                        </div>
                    </div>
                </div>
            </div>
