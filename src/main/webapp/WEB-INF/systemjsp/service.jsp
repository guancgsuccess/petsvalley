...
<head>
    ...
    <script>
        var url_getservice = '${pageContext.request.contextPath}/service/get';
        var url_getservicelist ='${pageContext.request.contextPath}/service/getAllByPageManage';
        //页面加载后，首先加载所有的消息数据
        window.onload=function() {
            refreshServiceList(url_getservicelist);
        ...
        //加载消息数据
        var xhr = null;
         function refreshServiceList(url_getservicelist) {
            var objArr = null;
            ...
            xhr.open("GET", url_getservicelist, true);
            xhr.setRequestHeader("Content-Type", "application/json;charset=utf-8");
            xhr.onreadystatechange = loadServices;
            xhr.send();
        }
       function delforService(obr1){
            var url_delservice = '${pageContext.request.contextPath}/service/delete/msgid/'+obr1;
            if (window.XMLHttpRequest) {
                xhr = new XMLHttpRequest();
            } else {
                xhr = new ActiveXObject('Microsoft.XMLHTTP');
            }
            xhr.open("GET", url_delservice, true);
           //xhr.setRequestHeader("X-Requested-With","XMLHttpRequest");
            xhr.setRequestHeader("Content-Type", "application/json;charset=utf-8");
           // xhr.onreadystatechange = refreshServiceList(url_getServicelist) ;
           window.location="${pageContext.request.contextPath}/service2";
            xhr.send();
        }
       function loadServices() {
            if (xhr.readyState == 4 && xhr.status == 200) {
                ...
                for (var i = 0; i < objArr.length; i++) {
                    tbody.innerHTML += ('<tr><td>' + objArr[i].serviceId + '</td>' +
                        '<td >' +objArr[i].serviceTitle + '</td>' +
                        '<td >' +objArr[i].serviceContent + '</td>' +
                        '<td>' +objArr[i].serviceReward +'</td>' +
                        '<td>' +objArr[i].serviceNo+'</td>'+
                        '<td>' +strFormat(objArr[i].serviceTime) +'</td>' +
                        '<td><a id="btnDel"  style="cursor: pointer" onclick="delforService('+ objArr[i].serviceId +')">删除</a>  &nbsp;&nbsp;&nbsp;&nbsp;  ' +
                        ' <a id="btnModif" name="' + objArr[i].serviceContent+""+'"  style="cursor: pointer" onclick="get33Service(this)" data-toggle="modal" data-target="#modalUpdateEmp">查看</a></td></tr>');
                }
                var pagearea=document.getElementById("pagearea");
                pagearea.innerHTML="";
                if(page.currentPageCode!=1){
                    var url_page=url_getservicelist+"/page/"+(page.currentPageCode-1);
                    var href='javascript:refreshServiceList("'+url_page+'")';
        ...
        function get33Service(msg) {
            document.getElementById("myService").innerText=msg.name;
        }

        function  href2page() {
            var url2page=url_getservicelist+"/page/"+(document.getElementById("topage").value);
            refreshServiceList(url2page);
        }
        ...
        function selectbydate() {
            ...
            var url_getservicelistbydate ='${pageContext.request.contextPath}/service/getAllByPagedateManage/?from1='+from1+'&to='+to;
            refreshServiceList(url_getservicelistbydate);
        }

        function addpet() {
            var title= document.getElementById("senduserid").value;
            var servicedetails= document.getElementById("addpetgender").value;
            var reward= document.getElementById("punishmentcontent").value;
            var peoplenum= document.getElementById("addpetagetime").value;
            if(title==""||servicedetails==""||reward==""||peoplenum==""){
                alert("信息不完整哦");
                return;
            }else {
                var url_addpet= '${pageContext.request.contextPath}/service/insert';
                if (window.XMLHttpRequest) {
                    xhr = new XMLHttpRequest();
                } else {
                    xhr = new ActiveXObject('Microsoft.XMLHTTP');
                }
                var fd="serviceTitle="+title+"&serviceContent="+servicedetails+"&serviceReward="+reward+"&serviceNo="+peoplenum;
        ...
        <div style="display: inline-block;width:79% !important;margin-right: -16%">
            <div class="col-md-4 column" style="background-color:whitesmoke;margin-top: 40px;width: 79%">
                <div style="margin-top: 25px;">
                    ...
                    <label><a style="cursor: pointer"  data-toggle="modal" data-target="#addpetdiv">发布志愿服务</a></label>
                </div>
            </div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                        <div class="form-group">
                           <table>
                               <tr>
                                   <td style="text-indent: 50px" id="myService"></td>
                               </tr>
                           </table>
                        </div>
                        <div class="form-group">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="form-group" style="text-align: center">
                        <form>
                            服务主题:<input type="text"  placeholder="请输入服务主题" id="senduserid" class="inpput"/><br/>
                            服务详情:<input type="text"  placeholder="请输入服务详情" id="addpetgender" class="inpput"/><br/>
                            积分奖励:<input type="text" id="addpetagetime" placeholder="请输入积分奖励" class="inpput"  ><br/>
                            招募人数:<input  type="text" placeholder="请输入招募人数" id="punishmentcontent" class="inpput"/><br/>
                        </form>
                    </div>
                    <div class="form-group">
                        <div class="col-sm-offset-2 col-sm-10" style="text-align: center">
                            <button type="button" class="btn btn-default" data-dismiss="modal">取消发布</button><button type="button" onclick="addpet()" class="btn btn-primary">确认发布</button>
                        </div>
                    </div>
                </div>
            </div>
