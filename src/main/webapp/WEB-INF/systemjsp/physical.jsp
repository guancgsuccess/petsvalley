...
<head>
    ...
    <script>
        var url_getphysical = '${pageContext.request.contextPath}/physical/get';
        var url_getphysicallist ='${pageContext.request.contextPath}/physical/getAllByPageManage';
        //页面加载后，首先加载所有的消息数据
        window.onload=function() {
            refreshPhysicalList(url_getphysicallist);
        ...
        //加载消息数据
        var xhr = null;
         function refreshPhysicalList(url_getphysicallist) {
            var objArr = null;
            ...
            xhr.open("GET", url_getphysicallist, true);
            xhr.setRequestHeader("Content-Type", "application/json;charset=utf-8");
            xhr.onreadystatechange = loadPhysicals;
            xhr.send();
        }
       function delforPhysical(obr1){
            var url_delphysical = '${pageContext.request.contextPath}/physical/delete/msgid/'+obr1;
            if (window.XMLHttpRequest) {
                xhr = new XMLHttpRequest();
            } else {
                xhr = new ActiveXObject('Microsoft.XMLHTTP');
            }
            xhr.open("GET", url_delphysical, true);
           //xhr.setRequestHeader("X-Requested-With","XMLHttpRequest");
            xhr.setRequestHeader("Content-Type", "application/json;charset=utf-8");
           window.location="${pageContext.request.contextPath}/physical";
            xhr.send();
        }
       function loadPhysicals() {
            if (xhr.readyState == 4 && xhr.status == 200) {
                ...
                for (var i = 0; i < objArr.length; i++) {
                    tbody.innerHTML += ('<tr><td>' + objArr[i].phyId + '</td>' +
                        '<td ><a style="cursor: pointer"onclick="getPetById(this)"data-toggle="modal" data-target="#petDetail">'+objArr[i].petId+'</a></td>' +
                        '<td >' + objArr[i].phyLoc + '</td>' +
                        '<td >' + objArr[i].phyType + '</td>' +
                        '<td >' + objArr[i].phyMessage + '</td>' +
                        '<td>' +strFormat(objArr[i].phyTime) +'</td>' +'<td ><a style="cursor: pointer"onclick="getCustomerById(this)"data-toggle="modal" data-target="#customerDetail">'+objArr[i].phySpare+'</a></td>' +
                        '<td><a id="btnDel"  style="cursor: pointer" onclick="delforPhysical('+ objArr[i].phyId +')">删除</a>  &nbsp;&nbsp;&nbsp;&nbsp;  ' +
                        ' <a id="btnModif" name="' + objArr[i].phyMessage+""+'"  style="cursor: pointer" onclick="get33Physical(this)" data-toggle="modal" data-target="#modalUpdateEmp">查看</a></td></tr>');
                }
                var pagearea=document.getElementById("pagearea");
                pagearea.innerHTML="";
                if(page.currentPageCode!=1){
                    var url_page=url_getphysicallist+"/page/"+(page.currentPageCode-1);
                    var href='javascript:refreshPhysicalList("'+url_page+'")';
        ...
        function get33Physical(msg) {
            document.getElementById("myPhysical").innerText=msg.name;
        }

        function  href2page() {
            var url2page=url_getphysicallist+"/page/"+(document.getElementById("topage").value);
            refreshPhysicalList(url2page);
        }
        ...
        function selectbydate() {
            ...
            var url_getphysicallistbydate ='${pageContext.request.contextPath}/physical/getAllByPagedateManage/?from1='+from1+'&to='+to;
            refreshPhysicalList(url_getphysicallistbydate);
        }
        function addpetdiv() {
        ...
        function addpet() {
            ...
            if(addpetgender==""||addpetname==""||addpetage==""||pettype==-1){
                alert("信息不完整哦");
                return;
            }else {
                var url_addpet= '${pageContext.request.contextPath}/physical/insert';
                if (window.XMLHttpRequest) {
                    xhr = new XMLHttpRequest();
                }else{
                    xhr = new ActiveXObject('Microsoft.XMLHTTP');
                }
                var fd="petId="+pettype+"&phyType="+addpetgender+"&phyLoc="+addpetage+"&phyMessage="+addpetname+"&phyTime="+addpetagetime;
        ...
        <div style="display: inline-block;width:79% !important;margin-right: -16%">
            <div class="col-md-4 column" style="background-color:whitesmoke;margin-top: 40px;width: 79%">
                <div style="margin-top: 25px;">
                    ...
                    <%--<label ><a style="cursor: pointer" onclick="addpetdiv()" data-toggle="modal" data-target="#addpetdiv">添加体检记录</a></label>--%>
                </div>
            </div>
...
<!-- 修改框 -->
<div>
    <div class="modal fade" id="modalUpdateEmp" tabindex="-2" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true"  style="display:none" style="display:none">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                        <div class="form-group">
                           <table>
                               <tr>
                                   <td style="text-indent: 50px" id="myPhysical"></td>
                               </tr>
                           </table>
                        </div>
                        <div class="form-group">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="form-group" style="text-align: center">
                        <form>
                            选择宠物:<select id="pettype" style="width: 80%" >
                            <option value="-1">请选择您的宠物</option>
                        </select><br/>
                            体检类型:<input type="text"  placeholder="请输入体检类型" id="addpetgender" class="inpput"/><br/>
                            体检时间:<input type="text" id="addpetagetime" placeholder="请选择体检时间" class="inpput"  ><br/>
                            体检地点:<input  type="text" placeholder="请输入体检地点" id="addpetage" class="inpput"/><br/>
                            体检详情:<input  type="text" placeholder="请输入详细信息" id="addpetname" class="inpput"/><br/>
                        </form>
                    </div>
                    <div class="form-group">
