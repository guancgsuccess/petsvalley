...
<head>
    ...
    <script>
        ...
        var url_getapplylist ='${pageContext.request.contextPath}/apply/getAllByPage';
        //页面加载后，首先加载所有的消息数据
       ...
       function delforApply(obr1){
           ...
           window.location="${pageContext.request.contextPath}/apply";
            xhr.send();
        }
       function loadApply() {
            if (xhr.readyState == 4 && xhr.status == 200) {
                ...
                for (var i = 0; i < objArr.length; i++) {
                    if(objArr[i].serviceId==null)objArr[i].serviceId="无";
                    ...
                    tbody.innerHTML += ('<tr><td>' + objArr[i].applyId + '</td>' +
                        '<td >' + objArr[i].serviceId + '</td>' +
                        '<td>' +objArr[i].fosterId   +'</td>'+'<td>' +strFormat(objArr[i].applyTime) +'</td>' +'<td>' +objArr[i].applySchedule +'</td>' + '<td >' + objArr[i].applyClassify + '</td>' +
                        '<td><a id="btnDel"  style="cursor: pointer" onclick="delforApply('+ objArr[i].applyId +')">删除</a>&nbsp;&nbsp;&nbsp;' +
                        '<a id="btnModif"   style="cursor: pointer" onclick="delforApply('+ objArr[i].applyId +')" <!--data-toggle="modal" data-target="#modalUpdateEmp"-->撤销</a></td></tr>');
                }
        ...
        function selectbydate() {
            ...
            var url_getapplylistbydate ='${pageContext.request.contextPath}/apply/getAllByPagedate/?from1='+from1+'&to='+to;
