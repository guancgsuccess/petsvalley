...
<head>
    ...
    <script>
        var url_getmatch = '${pageContext.request.contextPath}/match/get';
        var url_getmatchlist ='${pageContext.request.contextPath}/match/getAllByPage';
        //页面加载后，首先加载所有的消息数据
        window.onload=function() {
            refreshMatchList(url_getmatchlist);
        ...
        //加载消息数据
        var xhr = null;
         function refreshMatchList(url_getmatchlist) {
            var objArr = null;
            ...
            xhr.open("GET", url_getmatchlist, true);
            xhr.setRequestHeader("Content-Type", "application/json;charset=utf-8");
            xhr.onreadystatechange = loadMatchs;
            xhr.send();
        }
       function delforMatch(obr1){
            var url_delmatch = '${pageContext.request.contextPath}/match/delete/msgid/'+obr1;
            if (window.XMLHttpRequest) {
                xhr = new XMLHttpRequest();
            } else {
                xhr = new ActiveXObject('Microsoft.XMLHTTP');
            }
            xhr.open("GET", url_delmatch, true);
           //xhr.setRequestHeader("X-Requested-With","XMLHttpRequest");
            xhr.setRequestHeader("Content-Type", "application/json;charset=utf-8");
           window.location="${pageContext.request.contextPath}/match";
            xhr.send();
        }
       function loadMatchs() {
            if (xhr.readyState == 4 && xhr.status == 200) {
                ...
                for (var i = 0; i < objArr.length; i++) {
                    if(objArr[i].couplePetId==null)objArr[i].couplePetId="无";
                    if(objArr[i].coupleCustId==null)objArr[i].coupleCustId="无";
                    if(objArr[i].partnerPetId==null)objArr[i].partnerPetId="无";
                    if(objArr[i].partnerCustId==null)objArr[i].partnerCustId="无";
                    tbody.innerHTML += ('<tr><td>' + objArr[i].macId + '</td>' +
                        '<td >' + objArr[i].petId + '</td>' +
                        '<td >' + objArr[i].couplePetId + '</td>' +
                        '<td >' + objArr[i].coupleCustId + '</td>' +
                        '<td >' + objArr[i].partnerPetId + '</td>' +
                        '<td >' + objArr[i].partnerCustId + '</td>' +
                        '<td><a id="btnDel"  style="cursor: pointer" onclick="delforMatch('+ objArr[i].macId +')">解除匹配关系</a></td></tr>');
                }
                var pagearea=document.getElementById("pagearea");
                pagearea.innerHTML="";
                if(page.currentPageCode!=1){
                    var url_page=url_getmatchlist+"/page/"+(page.currentPageCode-1);
                    var href='javascript:refreshMatchList("'+url_page+'")';
        ...
        function get33Match(msg) {
            document.getElementById("myMatch").innerText=msg.name;
        }

        function  href2page() {
            var url2page=url_getmatchlist+"/page/"+(document.getElementById("topage").value);
            refreshMatchList(url2page);
        }
        ...
        function selectbydate() {
            ...
            var url_getmatchlistbydate ='${pageContext.request.contextPath}/match/getAllByPagedate/?from1='+from1+'&to='+to;
            refreshMatchList(url_getmatchlistbydate);
        }
