...
<head>
    ...
    <script>
        var url_getclaim = '${pageContext.request.contextPath}/claim/get';
        var url_getclaimlist ='${pageContext.request.contextPath}/claim/getAllByPage';
        //页面加载后，首先加载所有的消息数据
        window.onload=function() {
            refreshClaimList(url_getclaimlist);
        ...
        //加载消息数据
        var xhr = null;
         function refreshClaimList(url_getclaimlist) {
            var objArr = null;
            ...
            xhr.open("GET", url_getclaimlist, true);
            xhr.setRequestHeader("Content-Type", "application/json;charset=utf-8");
            xhr.onreadystatechange = loadClaim;
            xhr.send();
        }
       function delforClaim(obr1){
            var url_delClaim = '${pageContext.request.contextPath}/claim/delete/msgid/'+obr1;
            if (window.XMLHttpRequest) {
                xhr = new XMLHttpRequest();
            } else {
                xhr = new ActiveXObject('Microsoft.XMLHTTP');
            }
            xhr.open("GET", url_delclaim, true);
           //xhr.setRequestHeader("X-Requested-With","XMLHttpRequest");
            xhr.setRequestHeader("Content-Type", "application/json;charset=utf-8");
           window.location="${pageContext.request.contextPath}/claim";
            xhr.send();
        }
       function loadClaim() {
            if (xhr.readyState == 4 && xhr.status == 200) {
                ...
                for (var i = 0; i < objArr.length; i++) {
                    if(objArr[i].claimPlace==null)objArr[i].claimPlace="无";
                    if(objArr[i].claimMessage==null)objArr[i].claimMessage="无";
                    if(objArr[i].claimPhone==null)objArr[i].claimPhone="无";
                    tbody.innerHTML += ('<tr><td>' + objArr[i].claimId + '</td>'+
                        '<td >' + objArr[i].claimPlace + '</td>'+
                        '<td>' +strFormat(objArr[i].claimTime)+'</td>'+'<td>' +objArr[i].claimMessage+'</td>'+'<td>' +objArr[i].claimPhone+'</td>'+
                        '<td><a id="btnDel"  style="cursor: pointer" onclick="delforClaim('+ objArr[i].claimId +')">删除</a>&nbsp;&nbsp;&nbsp;' +
                        '<a id="btnModif"   style="cursor: pointer" name=" 发现地点:'+objArr[i].claimPlace+';详细信息:'+objArr[i].claimMessage+';联系电话:'+objArr[i].claimPhone+'" onclick="get33Claim(this)" data-toggle="modal" data-target="#modalUpdateEmp">查看</a></td></tr>');
                }
                var pagearea=document.getElementById("pagearea");
                pagearea.innerHTML="";
                if(page.currentPageCode!=1){
                    var url_page=url_getclaimlist+"/page/"+(page.currentPageCode-1);
                    var href='javascript:refreshclaimList("'+url_page+'")';
        ...
        function get33Claim(msg) {
            document.getElementById("myClaim").innerText=msg.name;
        }

        function  href2page() {
            var url2page=url_getclaimlist+"/page/"+(document.getElementById("topage").value);
            refreshClaimList(url2page);
        }
        ...
        function selectbydate() {
            ...
            var url_getclaimlistbydate ='${pageContext.request.contextPath}/claim/getAllByPagedate/?from1='+from1+'&to='+to;
            refreshClaimList(url_getclaimlistbydate);
        }
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                        <div class="form-group">
                           <table>
                               <tr>
                                   <td style="text-indent: 50px" id="myClaim"></td>
                               </tr>
                           </table>
                        </div>
                        <div class="form-group">
