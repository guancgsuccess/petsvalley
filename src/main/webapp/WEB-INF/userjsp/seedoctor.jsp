...
<head>
    ...
    <script>
        var url_getseedoctor = '${pageContext.request.contextPath}/seedoctor/get';
        var url_getseedoctorlist ='${pageContext.request.contextPath}/seedoctor/getAllByPage';
        //页面加载后，首先加载所有的消息数据
        window.onload=function() {
            refreshSeedoctorList(url_getseedoctorlist);
        ...
        $(function() {
            $( "#addpetage" ).datepicker();
            $( "#addpetage" ).datepicker( "option", "dateFormat","yy-mm-dd" );
        });

        //加载消息数据
        var xhr = null;
         function refreshSeedoctorList(url_getseedoctorlist) {
            var objArr = null;
            ...
            xhr.open("GET", url_getseedoctorlist, true);
            xhr.setRequestHeader("Content-Type", "application/json;charset=utf-8");
            xhr.onreadystatechange = loadSeedoctors;
            xhr.send();
        }
       function delforSeedoctor(obr1){
            var url_delSeedoctor = '${pageContext.request.contextPath}/seedoctor/delete/msgid/'+obr1;
            if (window.XMLHttpRequest) {
                xhr = new XMLHttpRequest();
            } else {
                xhr = new ActiveXObject('Microsoft.XMLHTTP');
            }
            xhr.open("GET", url_delseedoctor, true);
           //xhr.setRequestHeader("X-Requested-With","XMLHttpRequest");
            xhr.setRequestHeader("Content-Type", "application/json;charset=utf-8");
           // xhr.onreadystatechange = refreshSeedoctorList(url_getseedoctorlist) ;
           window.location="${pageContext.request.contextPath}/seedoctor";
            xhr.send();
        }
       function loadSeedoctors() {
            if (xhr.readyState == 4 && xhr.status == 200) {
                ...
                for (var i = 0; i < objArr.length; i++) {
                    tbody.innerHTML += ('<tr><td>' + objArr[i].docId + '</td>' +
                        '<td >' + objArr[i].docPetId + '</td>' +
                        '<td >' + objArr[i].docLoc + '</td>' +
                        '<td>' +strFormat(objArr[i].docTime) +'</td>' +
                        '<td>' +objArr[i].docDetalis+'</td>' +
                        '<td><a id="btnDel"  style="cursor: pointer" onclick="delforSeedoctor('+ objArr[i].docId +')">删除</a>  &nbsp;&nbsp;&nbsp;&nbsp;  ' +
                        ' <a id="btnModif" name="' + objArr[i].docDetalis+""+'"  style="cursor: pointer" onclick="get33Seedoctor(this)" data-toggle="modal" data-target="#modalUpdateEmp">查看</a></td></tr>');
                }
                var pagearea=document.getElementById("pagearea");
                pagearea.innerHTML="";
                if(page.currentPageCode!=1){
                    var url_page=url_getseedoctorlist+"/page/"+(page.currentPageCode-1);
                    var href='javascript:refreshSeedoctorList("'+url_page+'")';
        ...
        function get33Seedoctor(msg) {
            document.getElementById("mySeedoctor").innerText=msg.name;
        }

        function  href2page() {
            var url2page=url_getseedoctorlist+"/page/"+(document.getElementById("topage").value);
            refreshSeedoctorList(url2page);
        }
        ...
        function selectbydate() {
            ...
            var url_getseedoctorlistbydate ='${pageContext.request.contextPath}/seedoctor/getAllByPagedate/?from1='+from1+'&to='+to;
            refreshSeedoctorList(url_getseedoctorlistbydate);
        }
        function addpetdiv() {
        ...
        function addpet() {
            ...
            if(addpetgender==""||addpetname==""||addpetage==""||pettype==-1){
                ...
                var url_addpet= '${pageContext.request.contextPath}/seedoctor/insert';
                if (window.XMLHttpRequest) {
                    xhr = new XMLHttpRequest();
                } else {
                    xhr = new ActiveXObject('Microsoft.XMLHTTP');
                }
                var fd="docPetId="+pettype+"&docLoc="+addpetgender+"&docTime="+addpetage+"&docDetalis="+addpetname;
        ...
        <div style="display: inline-block;width:79% !important;margin-right: -16%">
            <div class="col-md-4 column" style="background-color:whitesmoke;margin-top: 40px;width: 79%">
                <div style="margin-top: 25px;">
                    ...
                    <label ><a style="cursor: pointer" onclick="addpetdiv()" data-toggle="modal" data-target="#addpetdiv">添加就医记录</a></label>
                </div>
            </div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                        <div class="form-group">
                           <table>
                               <tr>
                                   <td style="text-indent: 50px" id="mySeedoctor"></td>
                               </tr>
                           </table>
                        </div>
                        <div class="form-group">
                ...
                <div class="modal-body">
                    <div class="form-group" style="text-align: center">
                        <form>
                            ...
                            就医地点:<input type="text"  placeholder="请输入就医地点" id="addpetgender" class="inpput"/><br/>
                            就医时间:<input type="text" id="addpetage" placeholder="请选择就医时间" class="inpput"  ><br/>
                            就医详情:<input  type="text" placeholder="请输入详细信息" id="addpetname" class="inpput"/><br/>
                        </form>
                    </div>
                    <div class="form-group">
