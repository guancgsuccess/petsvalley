...
<head>
    ...
    <script>
        var url_getfindPet = '${pageContext.request.contextPath}/findPet/get';
        var url_getfindPetlist ='${pageContext.request.contextPath}/findPet/getAllByPage';
        //页面加载后，首先加载所有的消息数据
        window.onload=function() {
            refreshFindPetList(url_getfindPetlist);
        ...
        //加载消息数据
        var xhr = null;
         function refreshFindPetList(url_getfindPetlist) {
            var objArr = null;
            ...
            xhr.open("GET", url_getfindPetlist, true);
            xhr.setRequestHeader("Content-Type", "application/json;charset=utf-8");
            xhr.onreadystatechange = loadFindPet;
            xhr.send();
        }
       function delforFindPet(obr1){
            var url_delfindPet = '${pageContext.request.contextPath}/findPet/delete/msgid/'+obr1;
            if (window.XMLHttpRequest) {
                xhr = new XMLHttpRequest();
            } else {
                xhr = new ActiveXObject('Microsoft.XMLHTTP');
            }
            xhr.open("GET", url_delfindPet, true);
           //xhr.setRequestHeader("X-Requested-With","XMLHttpRequest");
            xhr.setRequestHeader("Content-Type", "application/json;charset=utf-8");
           window.location="${pageContext.request.contextPath}/findPet";
            xhr.send();
        }
       function loadFindPet() {
            if (xhr.readyState == 4 && xhr.status == 200) {
                ...
                for (var i = 0; i < objArr.length; i++) {
                    tbody.innerHTML += ('<tr>'+
                        ...
                        '<td>' +strFormat(objArr[i].lostTime) +'</td>'
                        + '<td>'+objArr[i].lostMessage+'</td>'
                        +'<td>' +objArr[i].lostPlace +'</td>'
                        + '<td>' + objArr[i].fpReward + '</td>'+ '<td >' + objArr[i].fpPhone + '</td>' +
                        '<td><a id="btnDel"  style="cursor: pointer" onclick="delforFindPet('+ objArr[i].aplId +')">删除</a>  &nbsp;&nbsp;&nbsp;&nbsp;  ' +
                        '<a id="btnModif" name="详细信息:' + objArr[i].lostMessage+';丢失地点:' + objArr[i].lostPlace+';联系方式:' + objArr[i].fpPhone+'"  style="cursor: pointer" onclick="get33FindPet(this)" data-toggle="modal" data-target="#modalUpdateEmp">查看</a></td></tr>');
                }
                var pagearea=document.getElementById("pagearea");
                pagearea.innerHTML="";
                if(page.currentPageCode!=1){
                    var url_page=url_getfindPetlist+"/page/"+(page.currentPageCode-1);
                    var href='javascript:refreshFindPetList("'+url_page+'")';
        ...
        function get33FindPet(msg) {
            document.getElementById("myFindPet").innerText=msg.name;
        }

        function  href2page() {
            var url2page=url_getfindPetlist+"/page/"+(document.getElementById("topage").value);
            refreshFindPetList(url2page);
        }
        ...
        function selectbydate() {
            ...
            var url_getfindPetlistbydate ='${pageContext.request.contextPath}/findPet/getAllByPagedate/?from1='+from1+'&to='+to;
            refreshFindPetList(url_getfindPetlistbydate);
        }
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                        <div class="form-group">
                           <table>
                               <tr>
                                   <td style="text-indent: 50px" id="myFindPet"></td>
                               </tr>
                           </table>
                        </div>
                        <div class="form-group">
